 ...
 * @since 1.5
 */
class TrackerstatsModelTrackerstats extends JModelForm
{
	/**
	 * @since	1.6
	 */
	protected $view_item = 'trackerstats';
	 ...
	 * @var		string
	 */
	protected $_context = 'com_trackerstats.trackerstats';
	...
	protected function populateState()
	{
		...
		$this->setState('trackerstats.id', $pk);
		...
		if ((!$user->authorise('core.edit.state', 'com_trackerstats')) &&  (!$user->authorise('core.edit', 'com_trackerstats'))){
	...
	public function &getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('trackerstats.id');
		...
		if (!isset($this->_item[$pk])) {
			try
			{
				...
				$query->from('#__trackerstats_details AS a');
		...
		if ($this->_item[$pk])
		{
			if ($extendedData = $this->getTrackerstatsQuery($pk)) {
	...
	protected function getTrackerstatsQuery($pk = null)
	{
		...
		if ($pk) {
			...
			// If we are showing a trackerstats list, then the trackerstats parameters take priority
			// So merge the trackerstats parameters with the merged parameters
				if ($this->getState('params')->get('show_trackerstats_list')) {
					$registry = new JRegistry;
