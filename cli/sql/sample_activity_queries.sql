...
FROM eem_code_activity_detail AS a
JOIN eem_code_users AS u
ON a.jc_user_id = u.jc_user_id
JOIN eem_code_activity_types AS t
...
AND Date(DATE_ADD(now(), INTERVAL -21 DAY)) 
 ...
 AS four_weeks_ago
 FROM `eem_code_activity_detail` AS a
JOIN eem_users AS u ON u.id = a.user_id
JOIN eem_code_activity_types AS t ON t.activity_type = a.activity_type
...
AND Date(DATE_ADD(now(), INTERVAL -21 DAY)) 
 ...
 FROM `eem_code_tracker_issues` AS t
